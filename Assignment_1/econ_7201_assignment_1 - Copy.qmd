...
**Ans**. I have created the repository and pushed all the changes to my repository on github.com (https://github.com/Emmanuel-spec493/econ_3201).
...
```{r}
 hist(x,freq = FALSE,ylab = "Density", xlab = "$x$")    
...
Subset the data to include only observations from 2008. Search `?subset` in the console.

```{r}
 mpg_2008 <- subset(mpg, year == 2008)
```

(a) Calculate the maximum and minimum miles per gallon in city limits (`cty`). Seach `?min` in the console.
...
```{r}
 ...
 df$compact <- ifelse(df$class == "compact", 1, 0)
...
```{r}
  plot(df_2008$cty, df_2008$hwy, xlab = "city mpg (cty)" , ylab = "highway mpg (hwy)",col ="blue" )
```
...
```{r}
   plot(df_2008$cty, df_2008$hwy,
     xlab = "City MPG (cty)",
     ylab = "Highway MPG (hwy)",
     main = "City versus Highway fuel Efficiency (MPG)", ,col = "blue")
```
...
mtext("Figure 1 Shows the Fuel Efficiency for City Driving versus Highway Driving",
